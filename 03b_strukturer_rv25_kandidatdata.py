...
def get_kv_kandidatdata(from_path, to_path, valg, data_type):
    all_files = kombiner_resultater(from_path, to_path, "rv", "kandidat-data")
    valgforbund_data = []
    kandidat_data = []
        ...
        try:
            ...
            for valgforbund in data['Valgforbund']:
                valgforbund_data.append({
                    'region': data['Region'],
                    'region_dagi_id': data['RegionDagiId'],
...
data = get_kv_kandidatdata(from_path, to_path, "rv", "kandidat-data")
...
df_kandidat_data.to_csv("data/struktureret/rv25_kandidat_data.csv", index=False)
df_valgforbund_data.to_csv("data/struktureret/rv25_valgforbund_data.csv", index=False)
